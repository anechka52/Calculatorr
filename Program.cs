    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            while (work)
            {
                ...
                else if (double.IsInfinity(ConsoleCalculator.Count(numOperation, firstOperand, secondOperand))) Console.WriteLine($"\nРезультат: Слишком сложное число для простого калькулятора!");
                else
                    result = ConsoleCalculator.Count(numOperation, firstOperand, secondOperand);
                Console.WriteLine($"\nРезультат: {result}");
                while (true)
                {
                    try
